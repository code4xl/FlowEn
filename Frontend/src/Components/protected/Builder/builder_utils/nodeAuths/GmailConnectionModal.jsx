...
//     try {
//       // 🔥 NEW: Use Authorization Code Flow instead of Identity Services
//       const authData = await authService.initiateAuthFlow();
...
//       // ✅ SAME: Rest of the logic remains unchanged
//       onSuccess(nodeId, {
//         service: 'gmail',
//         provider: 'google',
//         connected: true,
...
//         credentials: {
//           access_token: authData.access_token,
//           refresh_token: authData.refresh_token, // ✅ Now guaranteed!
//           token_type: authData.token_type,
//           client_id: authData.client_id,
...
const GmailConnectionModal = ({ isOpen, onClose, onSuccess, nodeId, workflowState }) => {
  const [isConnecting, setIsConnecting] = useState(false);
  const [authService] = useState(new GmailAuthService());
  const [error, setError] = useState(null);
  const [connectedAccount, setConnectedAccount] = useState(null);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Save workflow state and redirect
      await authService.initiateAuthFlow(nodeId, workflowState);
      
    } catch (error) {
      console.error('❌ Gmail authentication error:', error);
      setError(`Failed to connect to Gmail: ${error.message}`);
      setIsConnecting(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[var(--card-bg)] rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
        ...
        {connectedAccount ? (
          ...
          <div className="space-y-4">
            <p className="text-[var(--text-secondary)] mb-4">
              You'll be redirected to Google to authorize access. Your workflow will be preserved and you'll return here automatically.
            </p>
